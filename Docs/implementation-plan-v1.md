...
> **原则**：仅解决影响可玩性和可用性的体验问题，不添加新功能。
> 详细设计参考 [v1_enhancement.md](v1_enhancement.md)，本次实施 G1-G4/G6-G9/G14/G26 共 10 项。

### 6.5.1 任务清单

| # | 任务 | 差距项 | 详情 |
|---|------|--------|------|
| 4.6.1 | All-in 二次确认 | G3 | 全押前弹出 Alert.alert 确认对话框 |
| 4.6.2 | 按钮触摸反馈 | G2 | Pressable + Animated scale(0.95) 按压效果 |
| 4.6.3 | Loading 状态 | G7 | 创建/加入房间按钮增加 ActivityIndicator |
| 4.6.4 | 结算自动消失 | G26 | 3 秒后自动 resetHand，保留手动"继续"按钮 |
| 4.6.5 | 阶段进度条 | G6 | 新增 StageProgressBar 组件，显示当前游戏阶段 |
| 4.6.6 | 大尺寸底牌展示 | G14 | 牌桌底部独立区域展示 68×96px 底牌 |
| 4.6.7 | 加注面板重设计 | G4 | 底部滑出面板 + 快捷金额按钮（½底池/底池/2x/All-in）|
| 4.6.8 | 牌面角标布局 | G8 | CardView 重写为角标布局 + 中央大花色 + 纸张质感 |
| 4.6.9 | 座位组件重设计 | G9 | 圆形头像区域 + 弃牌覆盖文字 + ALL IN 标签 |
| 4.6.10 | 操作倒计时环 | G1 | react-native-svg 环形进度条 + 颜色渐变 |

### 6.5.2 新增依赖
- `react-native-svg` — 倒计时环形进度条

### 6.5.3 验收标准
- 所有按钮有按压缩放反馈
- All-in 需二次确认才能执行
- 创建/加入房间时按钮显示 Loading 状态
- 结算 3s 后自动进入下一手
- 底部有游戏阶段进度条
- 自己底牌以大尺寸独立区域展示
- 加注面板包含快捷金额按钮
